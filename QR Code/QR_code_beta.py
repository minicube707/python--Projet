...
def encode(message,  total_data_codewords):
    ...
    if all(digit in list(numeric) for digit in list(message)):
        ...
        mode = 1
        print("mode: numeric")
        print("mode: ", format(mode, "04b"))
        print("count: ", format(count, "010b"))
...
message_poly =  encode("HELLO WORLD", 26-nb_correction)
...
#Create the grid
save_grid = make_QR_Code(21, final_message, forbidden_cell)
...
for num_mask in range(8):
    ...
    for i in range(len_qr_code):
        for j in range(len_qr_code):
            ...
            if not any(np.array_equal(sub, np.array([i, j]))  for sub in forbidden_cell) and mask(num_mask, i, j):
                grid[j, i] = 1 - grid[j, i]
...
while len(format_string) < 10:
    format_string = np.append(format_string, 0)
