...
def solve_tsp(WIN, list_point, order_point, distance, path, start_point, previous_point, nb_combinaison, best_distance, best_order, initial_list, name_list):
    ...
    for index, point in enumerate (list_point):
        ...
        if len(new_list_point) == 0:
            ...
            if new_path[-1] >  new_path[1]:
                ...
                draw_best_distance(best_distance)
                pygame.display.flip()
            

        #Call the function recursively
        nb_combinaison, best_distance, best_order = solve_tsp(WIN, new_list_point, new_order_point, new_distance, new_path.astype(int), start_point, list_point[index], nb_combinaison, best_distance, best_order.astype(int), initial_list, name_list)
...
def draw_best_distance(best_distance):
    # Création de la surface de texte
    font = pygame.font.Font(None, 30)
    text_surface = font.render("Worst distance {:,.0f}".format(best_distance).replace(",", " "), True, WHITE)
    text_position = (10, HEIGHT - text_surface.get_height() - 10)  # 10px de marge
    WIN.blit(text_surface, text_position)
...
while True:
    ...
    if not find_shortess_path:
        ...
        Y = np.random.randint(0, HEIGHT, number_point)
        ...
        _, best_distance, best_order = solve_tsp(WIN, list_point, order_point, 0, path, start_point, previous_point, nb_combinaison, best_distance, best_order, initial_list, name_list)
