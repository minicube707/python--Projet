...
def soustraction(coords1 , coords2):
    ...
    for coord1 in coords1:
        ...
        for coord2 in matching_coords:
            ...
            result.append(y1 - y2 -1)  # On conserve le même X
...
def find_position(grid, rows, cols, next_figure, figure, next_num, num, nb_state, next_nb_state):
    ...
    return best_state, best_delta_x, intial_figure

def add_move(rows, all_figure, best_state, best_delta_x, figure, state, num):
    ...
    if state == best_state and min_x == best_delta_x:
        delta_y = 1
    ...
    for land_figure in all_figure:
        for bloc_A in land_figure:
            for bloc in figure:
                ...
                bloc_B = np.array([bloc[0] + delta_y, bloc[1]])
    ...
    for bloc in figure:
        ...
        if max_y + delta_y < rows and not contact_y:
                new_bloc = np.append(new_bloc, bloc[0] + delta_y)
        else:
    ...
    return new_figure, new_state, is_fall


def place_figure(rows, all_figure, best_state, best_delta_x, figure, state, num, start, delay):

    fall = 0
    contact_y = False   # the figure colide on the y axis ?

    new_figure, new_state, is_fall = add_move(rows, all_figure, best_state, best_delta_x, figure, state, num)
    
    max_y = np.max(new_figure[:, 0], 0)
    figure = new_figure
    ...
    for land_figure in all_figure:
        for bloc_A in land_figure:
            for bloc in figure:
                ...
                #If the bloc A and B are the same there is a colision on the y axis
                bloc_B = np.array([bloc[0] + fall, bloc[1]])
                if np.array_equal(bloc_A, bloc_B):
                    is_fall = False
                    contact_y = True             
    ...
    for bloc in figure:
        ...
        if max_y + fall < rows and not contact_y:
                new_bloc = np.append(new_bloc, bloc[0] + fall)
        else:
            new_bloc = np.append(new_bloc, bloc[0])
...
grid = np.zeros((rows, cols), np.int8)
best_state, best_delta_x, figure = find_position(grid, rows, cols, next_figure, figure, next_num, num, nb_state, next_nb_state)
...
while run:
    ...
    if not end:
        ...
        #If the figure fall
            ...
            figure, state, start, is_fall = place_figure(rows, all_figure, best_state, best_delta_x, figure, state, num, start, delay)
