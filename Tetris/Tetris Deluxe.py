...
#Load the best score
try:
    pickle_in = open("best_score.pickle", "rb")
    best_score = pickle.load(pickle_in)
except:
    best_score = 0
...
#Turn the figure
def turn_figure(figure, num, state, rows, cols):
    ...
    #Add the mouvement    
    if (0 <= min_x and max_x <= cols-1) and( max_y  < rows) and not contact:
        figure = test_figure

    return figure, state

#Move the figure
def move_figure(run, figure, start, delay, all_figure, num, state, start_press, rows, cols, pause, sound_mute):

    delta_x = 0     #Variation on the x axis 
    delta_y = 0     #Variation on the y axis 
    fall = 0

    #Boolean
    is_fall = True      # figure is fall ?
    contact_x = False   # the figure colide on the x axis ?
    ...
    if keys[pygame.K_UP] and time.time() >= (start_press + 0.2):
        start_press = time.time()
        figure, state = turn_figure(figure, num, state, rows, cols)
        moved = True
    ...
    if run:  
        ...
        for land_figure in all_figure:
            for bloc_A in land_figure:
                for bloc in figure:
                    ...
                    #If the bloc A and B are the same there is a colision on the x axis
        ...
        for bloc in figure:
            ...
            #verify if the figure don't get out the boder on the x axis
            new_bloc = np.array([], np.int8)
            if (0 <= min_x + delta_x and max_x + delta_x < cols) and not contact_x:
                new_bloc = np.append(new_bloc, bloc[0] + delta_x)
            else:
                new_bloc = np.append(new_bloc, bloc[0])

            #verify if the figure don't get out the boder on the x axis
            if max_y + delta_y + fall < rows and not contact_y:
...
def draw(WIN, HEIGHT, rows, cols, cell_size, x_offset, y_offset, all_figure, list_colour, colour, figure, next_num):
    ...
    text_best_score = FONT.render(f"Best Score: {best_score:,}".replace(",", " "), 1, WHITE)
    ...
    WIN.blit(text_best_score, (x_offset//2 - text_score.get_width()//2, 3*HEIGHT//4 - text_score.get_height() - text_best_score.get_height()))
...
def add_figure(list_num):
    ...
    return figure, colour, num, list_num
...
time_delay = 0.5        #The initial delay between each fall of the figure
delay = time_delay      #The delay between each fall of the figure, this will  be udate during the game
...
figure, colour, num, list_num = add_figure(list_num)
state = 0

next_figure, next_colour, next_num, list_num = add_figure(list_num)
next_state = 0
...
while run:
    ...
    if not end:
        ...
        #If the figure didn't fall and the delay is pass
            ...
            delay = time_delay - (tt_nb_line//10) * 0.05
        ...
        #If the figure fall
            ...
            run, figure, start, is_fall, state, start_press, moved, pause, sound_mute = move_figure (run, figure, start, delay, all_figure, num, state, start_press, rows, cols, pause, sound_mute)
    ...
    #If the game is over
        ...
        if score > best_score:
            best_score = score
            with open('best_score.pickle', 'wb') as file:
        ...
        text_score = FONT.render(f"Score: {score:,}".replace(",", " "), 1, WHITE)
        text_level = FONT.render("Level: " + str(level), 1, WHITE)
        text_best_score = FONT.render(f"Best Score: {score:,}".replace(",", " "), 1, WHITE)

        WIN.blit(game_over, (WIDTH//2 - game_over.get_width() // 2, HEIGHT//2))
        WIN.blit(text_level, (WIDTH//2 - text_level.get_width()// 2, HEIGHT//2 + game_over.get_height())) 
        WIN.blit(text_score, (WIDTH//2 - text_score.get_width()// 2, HEIGHT//2 + text_level.get_height() + game_over.get_height()))
        WIN.blit(text_best_score, (WIDTH//2 - text_score.get_width()// 2, HEIGHT//2 + text_level.get_height() + game_over.get_height() + text_score.get_height()))
