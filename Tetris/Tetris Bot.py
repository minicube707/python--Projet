...
def make_score(test_figure, test_grid):
    ...
    for bloc in list_bloc:
        ...
        for delta in list_delta:
            ...
            if (0 <= bloc[0] + delta < rows):
                if test_grid[bloc[0] + delta, bloc[1]] == 0:
        ...
        for delta in list_delta:
            ...
            if (0 <= bloc[1] + delta < cols):
                if test_grid[bloc[0], bloc[1] + delta] == 0:
    ...
    malus = 0
    list_free_air_bloc = list_air_bloc.copy()
    ...
    for air_bloc in list_air_bloc:
        ...
        for delta in list_delta:
            if (0 <= air_bloc[0] + delta < rows):
                if test_grid[air_bloc[0] + delta, air_bloc[1]] == 0:
                    surrounded = False

        for delta in list_delta:
                if (0 <= air_bloc[1] + delta < cols):
                    if test_grid[air_bloc[0], air_bloc[1] + delta] == 0:
                        surrounded = False

        if surrounded:
            list_free_air_bloc.remove((air_bloc))
            malus += (rows -  air_bloc[0])*2

    return test_grid, malus


def find_position(grid, rows, cols, figure, num, nb_state):
    ...
    for state in range(nb_state):
        ...
        width_figure = max_x - min_x

        #For every position on the x axis
        for delta_x in range(cols - width_figure):
            ...
            while not contact:
                ...
                for bloc in figure:
                    ...
                    if (bloc[1] + delta_y == rows):
                        contact = True

                    #Then verify if the figure touch an other figure
                    elif bloc[1] + delta_y >= 0: #avoid than the index is negative

                        #If there a bloc below 
                        if (grid[bloc[1] + delta_y, bloc[0] + delta_x - min_x] == 1):
                            nb_contact += 1
                            contact = True   
                ...
                if not contact:
                    ...
                    test_figure = np.array([], np.int8)
                    for bloc in figure:
                        test_bloc = np.array([], np.int8)
                        test_bloc = np.append(test_bloc, bloc[0] + delta_x - min_x)
                        test_bloc = np.append(test_bloc, bloc[1] + delta_y)
                        test_figure = np.append(test_figure, test_bloc)

                    delta_y +=1
                    test_figure = test_figure.reshape(4, 2)


            test_grid = grid.copy()
            test_grid, malus = make_score(test_figure, test_grid)
        ...
        figure, _ = turn_figure(figure, num, state)
...
grid = np.zeros((rows, cols), np.int8)
best_state, best_delta_x = find_position(grid, rows, cols, figure, num, nb_state)
